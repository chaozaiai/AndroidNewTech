...
public class BaseRecyclerHolder extends RecyclerView.ViewHolder {
    ...
    public BaseRecyclerHolder(View itemView, Context context) {
        super(itemView);
        //一般不会超过8个吧
        this.mViews = new SparseArray<>(18);
     ...
     * 为TextView设置字符串
     */
    public BaseRecyclerHolder setTextSpan(int viewId, SpannableString text) {
        TextView view = getView(viewId);
        view.setText(text);
        return this;
    }
     ...
     * 为TextView设置字符串
     */
    public BaseRecyclerHolder setTextColor(int viewId, int resId) {
        TextView view = getView(viewId);
...
//        else
//            try {
//                gongzhonghao bitmap = ImageCacheManager.getInstance().getBitmap(url);
//                if (bitmap!=null){
